...
const CountryStats = props => {
  ...
  async function getData() {
    if (props.country == 'TR') {
      ...
      const res = await axios(
        `https://corona.lmao.ninja/v2/countries/${props.country}`
      );
      const data = res.data;
      setStatsData(data);
      setLoading(false);
  ...
  return (
    <div>
      ...
      <div className='card-deck'>
              ...
              {props.country == 'TR' && !loading ? (
                <p className='card-text font-italic'>
                  ...
                  <span className='font-weight-bold'>
                    {numberFormatter(statsData.todayCases)}
                  </span>{' '}
                </p>
              ) : (
                ''
            ...
            <div className='card-body'>
              ...
              {props.country == 'TR' && !loading ? (
                <p className='card-text font-italic'>
                  <FormattedMessage id='today' /> :{' '}
                  <span className='font-weight-bold'>
                    {numberFormatter(statsData.todayDeaths)}
                  </span>{' '}
                </p>
              ) : (
                ''
            ...
            <div className='card-body'>
              ...
              {props.country == 'TR' && !loading ? (
                <p className='card-text font-italic'>
                  <FormattedMessage id='today' /> :{' '}
                  <span className='font-weight-bold'>
                    {numberFormatter(statsData.todayCritical)}
                  </span>{' '}
                </p>
              ) : (
                ''
            ...
            <div className='card-body'>
              ...
              {props.country == 'TR' && !loading ? (
                <p className='card-text font-italic'>
                  <FormattedMessage id='today' /> :{' '}
                  <span className='font-weight-bold'>
                    {numberFormatter(statsData.todayRecovered)}
                  </span>{' '}
                </p>
              ) : (
                ''
        ...
        <Row>
          <div className='card bg-secondary'>
            <div className='card-header font-weight-bold text-uppercase'>
              <FormattedMessage id='total_test' />
            </div>
            <div className='card-body'>
              {loading ? (
                <Spinner animation='border' />
              ...
              {props.country == 'TR' && !loading ? (
                <p className='card-text font-italic'>
                  <FormattedMessage id='today' /> :{' '}
                  <span className='font-weight-bold'>
                    {numberFormatter(statsData.todayTests)}
                  </span>{' '}
                </p>
              ) : (
                ''
          ...
          <div className='card bg-dark'>
            <div className='card-header font-weight-bold text-uppercase'>
              <FormattedMessage id='death_rate' />
            </div>
            <div className='card-body'>
              {loading ? (
                <Spinner animation='border' />
                ...
                <h3 className='card-title'>
                  {' '}
                  {calculateRate(statsData.deaths, statsData.cases)}
                </h3>
              )}
            </div>
          </div>
          <div className='card bg-lighter'>
            <div className='card-header font-weight-bold text-uppercase text-dark'>
            ...
            <div className='card-body'>
              {loading ? (
                ...
                <h3 className='card-title text-dark'>
                  {' '}
                  {calculateRate(statsData.recovered, statsData.cases)}
                </h3>
              )}
            </div>
          </div>
      ...
      <section className='my-4 align-left'></section>
      <br></br>
    </div>
  );
};
